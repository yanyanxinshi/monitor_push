...
> ⚠️ **安全提示**：本项目已完成敏感信息脱敏处理。在使用前，请务必阅读 [SECURITY_CHECKLIST.md](SECURITY_CHECKLIST.md) 确保您的配置安全。

基于 MTProto 协议的 Telegram 消息监控系统，专为 GitHub Actions 部署优化，可将私密群组消息转发至钉钉/飞书/企业微信。

## ✨ 核心特性

- 🔐 **StringSession 认证**：无需本地 `.session` 文件，完全基于环境变量
- ⏰ **智能时间窗口**：仅在北京时间 09:00-23:55 运行
- 📨 **历史消息补发**：自动补发运行间隙产生的消息，支持分页获取（无消息丢失）
- 🔄 **轻量级轮询**：每 5 分钟执行一次（整点 0、5、10、15... 分），抓完即退出
- 💾 **状态持久化**：自动保存并提交最后处理的消息 ID
- ⚡ **配额优化**：每月仅消耗约 360 分钟（节省 64% 配额）
- 🛡️ **并发控制**：自动排队机制，防止多任务冲突
- 🔥 **超时熔断**：3 分钟超时自动清理，让新任务接管
...
```mermaid
graph TD
    A[GitHub Actions 触发<br/>每 5 分钟<br/>整点 0、5、10、15... 分] --> B{检查工作时段}
    B -->|不在 09:00-23:55| C[安全退出]
    B -->|在工作时段| D[读取 last_id.txt]
    D --> E[连接 Telegram]
    E --> F[抓取历史消息<br/>分页获取所有新消息]
    F --> G{有新消息?}
    G -->|是| H[转发至 Webhook<br/>3秒/条]
    G -->|否| I[发送无消息提示]
    H --> J[更新 last_id.txt]
    I --> J
    J --> K[断开连接]
    K --> L[提交 last_id.txt]
    L --> M[结束<br/>等待下一个 5 分钟]
```

### 时间对齐机制

**关键特性**：即使脚本没有准时启动，也会在整点的 0、5、10、15... 分钟运行
...
09:05 - Cron 触发 ✅
09:10 - Cron 触发 ✅
09:12 - 假设 09:10 的任务延迟到现在才启动
09:15 - Cron 触发 ✅（不受 09:10 延迟影响）
09:20 - Cron 触发 ✅
```

**保证机制**：
- Cron 表达式：`0,5,10,15,20,25,30,35,40,45,50,55 1-15 * * *`
- 每小时固定在 0、5、10、15、20、25、30、35、40、45、50、55 分触发
- 不会因为某次延迟而影响后续触发时间
...
09:00 ─ 任务 A 启动
09:02 ─ 任务 A 仍在运行（处理大量消息）
09:03 ─ 任务 A 超时，被强制杀死
09:05 ─ 任务 B 启动（读取到 08:55 的 last_id）
09:06 ─ 任务 B 完成（一次性处理 08:55-09:05 的所有消息）
09:10 ─ 任务 C 启动（正常运行）
```

**关键保护**：
- `concurrency.cancel-in-progress: false` → 新任务排队，不会并行
- `timeout-minutes: 3` → 超时自动清理
- 无文件锁冲突，无数据丢失
...
1. **GitHub Actions 配额（已大幅优化）**：
   ...
   - **10分钟方案**：90 次/天 × 2 分钟 ≈ 180 分钟/月
   - **5分钟方案**：180 次/天 × 2 分钟 ≈ **360 分钟/月**
   - **节省 64% 配额！**
   - **已内置保护**：
     - 脚本层面：单次执行 < 3 分钟
     - Actions 层面：3 分钟强制超时（防止卡死）
     - 并发控制：自动排队，防止多任务冲突

2. **时间覆盖**：
   - 运行时段：北京时间 09:00-23:55（每 5 分钟一次，整点 0、5、10、15... 分）
   - **23:55-09:00 的消息**：次日 09:00 补发
   - 无数据丢失，最多延迟 9 小时 5 分钟
   - 如需 24 小时覆盖，可调整 Cron 为 `0,5,10,15,20,25,30,35,40,45,50,55 * * * *`（每月约 8640 分钟，超出免费额度）
