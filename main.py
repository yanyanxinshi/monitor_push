...
3. 支持历史消息补发和实时监听
4. 自动保存最后处理的消息 ID
5. 轻量级轮询模式，每 5 分钟执行一次（整点 0、5、10、15... 分）
...
class Config:
    ...
    # Webhook 频率限制（钉钉/飞书/企微限制：20条/分钟）
    WEBHOOK_SEND_INTERVAL = 3.0  # 发送间隔（秒），3秒 = 20条/分钟
    # 注意：每 5 分钟运行一次，单次最多处理约 100 条消息（5分钟 ≈ 300秒 / 3秒 = 100条）
...
async def run_monitor():
    ...
    设计理念：
    - 每 5 分钟执行一次（整点 0、5、10、15... 分），单次运行 < 3 分钟
    - 不需要长连接监听，不需要信号处理
    - "抓完就走"，简洁高效
    """
    # 1. 验证配置
...
if __name__ == '__main__':
    ...
    print("  运行模式：每 5 分钟执行一次（整点 0、5、10、15... 分）")
    print("=" * 60)
